 ...
 * # leaflet
 */
angular.module("appApp").directive("leaflet", [
  "leaflet",
  "_",
  "configuration",
  "$compile",
  ...
  function(
    ...
    return {
      ...
      link: function postLink(scope) {
        leaflet.MakiMarkers.accessToken =
          "pk.eyJ1IjoibWFyY29sYXJvc2EiLCJhIjoiY2lpbXZsNGU3MDFkYnVka25vZjZlYnl6dCJ9.TodPqRbm7MnCj2y-8D6UCA";
        ...
        scope.$on("zoom-to", function() {
          if (conf.latlng.lat && conf.latlng.lng) {
            scope.map.panTo(
              leaflet.latLng(
                parseFloat(conf.latlng.lat),
                parseFloat(conf.latlng.lng)
              )
            );
            scope.map.setZoom(8);
        ...
        angular.element(document.getElementById("map"))[0].style.height =
        ...
        scope.drawMarkers = function() {
          scope.markersByCode = {};
          scope.markerList = _.compact(
            _.map(scope.languages, function(l) {
              if (parseFloat(l.coords[0]) && parseFloat(l.coords[1])) {
                ...
                var marker = leaflet.marker(
                  new leaflet.LatLng(
                    parseFloat(l.coords[0]),
                    parseFloat(l.coords[1])
                  ),
                  { icon: icon }
                );
                ...
                if (!scope.compiledPopups[l.name]) {
                  var popup = $compile(
                    "<span><h4>" +
                      ...
                      "<br/> (" +
                      c +
                      " resources)</h4><br/><a href='' ng-click='moreInfo(\"" +
                      l.code +
          ...
          $timeout(function() {
            var group = new leaflet.featureGroup(scope.markerList);
