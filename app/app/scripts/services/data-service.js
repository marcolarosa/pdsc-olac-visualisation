...
angular.module("appApp").service("dataService", [
  "$http",
  "_",
  "configuration",
  "$rootScope",
  function($http, _, conf, $rootScope) {
    ...
    ds.mapLanguagesToCountries = function() {
      ...
      _.each(ds.datasets.countries, function(country) {
        _.each(country.language_data, function(language) {
          try {
            ds.datasets.languageToCountryMapping[language.code].push(
              country.name
            );
          } catch (e) {
            ds.datasets.languageToCountryMapping[language.code] = [
              country.name
            ];
          }
        });
    ...
    ds.filter = function(country, resource) {
      console.log(country, resource);
    };
    ...
    ds.filterByResource = function(resource) {
      ...
      if (_.isEmpty(ds.resourceFilters)) {
        ...
        languages = _.compact(
          _.map(ds.datasets.languages, function(l) {
            if (
              !_.isEmpty(
                _.intersection(_.keys(l.resources), ds.resourceFilters)
              )
            ) {
              return l;
            }
    ...
    ds.filterByCountry = function(country) {
      ...
      if (_.isEmpty(ds.countryFilter)) {
        ...
        countries = [country];
        languages = _.compact(
          _.map(ds.datasets.languages, function(language) {
            try {
              if (
                ds.datasets.languageToCountryMapping[language.code].indexOf(
                  country.name
                ) !== -1
              ) {
                return language;
